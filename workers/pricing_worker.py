...
async def _get_live_fx_rate() -> Decimal:
    ...
    --- MOCK IMPLEMENTATION ---
    In a production environment, this function would make a real API call to a
    reliable FX provider (e.g., using aiohttp). For this example, we simulate
    the call and return a mock rate.
    """
...
def _calculate_final_ngn(pva_usd_price: Decimal, fx_rate_to_use: Decimal) -> int:
    ...
    INTERNAL RULE (NOT SHOWN TO USERS)
    final_usd = pva_usd_price * 2
    final_ngn = final_usd * fx_rate
    """
    ...
    # Round up to the nearest 10 NGN for cleaner pricing (e.g., 1253.78 -> 1260)
...
async def get_final_ngn_price(country_id: str, service_id: str, number_type: str) -> tuple[
    Optional[int], Optional[str]]:
    """
    The main public function for this worker. It gets the final price in NGN,
    utilizing a cache to avoid repeated calculations.

    Returns: A tuple of (final_ngn_price, price_reference_id) or (None, None) on failure.
    """
    # 1. Check Cache
