...
def paginated_list_keyboard(items: list, prefix: str, page: int, total_pages: int, back_callback: str) -> InlineKeyboardMarkup:
    ...
    nav_buttons = []
    if page > 1:
        nav_buttons.append(InlineKeyboardButton(text="« Prev", callback_data=f"page:{prefix}:{page-1}"))
    if page < total_pages:
        nav_buttons.append(InlineKeyboardButton(text="Next »", callback_data=f"page:{prefix}:{page+1}"))
